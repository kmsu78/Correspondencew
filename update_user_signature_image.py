...
def update_database_schema():
    """تحديث مخطط قاعدة البيانات لإضافة حقل صورة التوقيع للمستخدمين"""
    ...
    try:
        # التحقق من وجود عمود signature_image في جدول user
        cursor.execute("PRAGMA table_info(user)")
        columns = cursor.fetchall()
        column_names = [column[1] for column in columns]
        
        # إضافة عمود signature_image إذا لم يكن موجودًا
        if 'signature_image' not in column_names:
            print("إضافة عمود signature_image إلى جدول user...")
            cursor.execute("ALTER TABLE user ADD COLUMN signature_image TEXT")
            ...
            print("العمود signature_image موجود بالفعل في جدول user")
            return True
